...
def is_microbiology_basic(text: str) -> bool:
    ...
    if any(h in lower for h in HUMAN_CLINICAL_HINTS):
        return False
    return contains_any(lower, MICROBIO_ORG_KEYWORDS) or contains_any(
        lower, MICROBIO_LAB_KEYWORDS
    )
...
def build_faiss_index(
    ...
    logger.info(
        f"Loaded {len(all_texts)} texts for indexing (after filtering={filter_pubmed})"
    )
    ...
    logger.info(
        f"Building FAISS index with batch size {batch_size} (this may take a while)..."
    )
        ...
        for i in range(0, len(all_texts), batch_size):
            ...
            if vectorstore is None:
                vectorstore = FAISS.from_texts(
                    texts=batch_texts, embedding=embeddings, metadatas=batch_metadata
                )
            else:
